...
#             Radiobutton(setup_window, text=difficulty, variable=difficulty_var, value=index).pack()

#         Button(setup_window, text="Confirm and Proceed", 
#                command=lambda: self.open_simulation_window(name_entry.get(), difficulties[difficulty_var.get()], setup_window)).pack(pady=20)
...
#         state_label = Label(simulation_window, text="State: Waiting")
...
#         Label(final_score_window, text=f"{name}, your final score: {self.total_score}").pack(pady=20)
#         Button(final_score_window, text="Next...", command=lambda: self.close_simulation(simulation_window, final_score_window)).pack(pady=20)
...
score = calcScore(imgPath, 3)
print(score)


locateReticule(imgPath)